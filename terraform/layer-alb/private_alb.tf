...
resource "aws_route53_record" "private_alb_dns" {
  ...
  name    = "private.${data.terraform_remote_state.layer-base.outputs.public_dns_zone}"
  type    = "CNAME"
  ttl     = "300"
...
resource "aws_lb_listener" "private_alb" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = "${data.terraform_remote_state.layer-eks.outputs.private-target-group}"
  }
}
