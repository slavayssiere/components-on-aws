...
resource "aws_route53_record" "public_alb_dns" {
  ...
  name    = "public.${data.terraform_remote_state.component-base.outputs.public_dns_zone}"
  type    = "CNAME"
  ttl     = "300"
...
resource "aws_lb_listener" "public_alb" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = "${data.terraform_remote_state.component-eks.outputs.public-target-group}"
  }
}
