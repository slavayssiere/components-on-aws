...
ssh -M -S my-ctrl-socket -fnNT -L 8443:k8s-master.slavayssiere.wescale:443 ec2-user@bastion.$PLATEFORM_NAME.$DNS_PUBLIC
...
ssh ec2-user@bastion.$PLATEFORM_NAME.$DNS_PUBLIC aws --region eu-west-1 eks update-kubeconfig --name eks-test-$PLATEFORM_NAME --role-arn arn:aws:iam::$ACCOUNT:role/bastion_role_$PLATEFORM_NAME

scp -r $WORKDIR/../terraform/component_eks/mon-network ec2-user@bastion.$PLATEFORM_NAME.$DNS_PUBLIC:
scp -r $WORKDIR/../terraform/component_eks/helm_values ec2-user@bastion.$PLATEFORM_NAME.$DNS_PUBLIC:
...
ssh ec2-user@bastion.$PLATEFORM_NAME.$DNS_PUBLIC ./eks-on-bastion.sh
